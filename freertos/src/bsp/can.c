...
{
	(void)flexcanState;//此处不使用驱动状态参数，如果需要可以根据实际情况进行处理
	(void)instance;   //此处不使用实例索引参数，如果需要可以根据实际情况进行处理
	(void)buffIdx;    //此处不使用缓冲区索引参数，如果需要可以根据实际情况进行处理
...
static void CAN0_Init(void)
{
	  ...
	  can_buff_config_t Rx_buffCfg =  {
	      .enableFD = false,        //是否使用CAN FD
	      .enableBRS = false,       //是否使用BRS (仅当enableFD为true时有效)
	      .fdPadding = 0U,          //当使用CAN FD且数据长度小于DLC指定的长度时，填充字节的值
	      .idType = CAN_MSG_ID_STD, //使用标准ID还是扩展ID
	      .isRemote = false         //是否配置为远程帧，远程帧是一种特殊的帧类型，用于请求数据而不携带数据本身  
	  };
...
void can_send(uint32_t id,uint8_t *data,uint8_t len)
{
	...
	if(STATUS_SUCCESS == CAN_Send(&can_pal1_instance, TX_MAILBOX_CAN0, &TxMsg))
	{
		num=10;
	}
