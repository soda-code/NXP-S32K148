...
void BSP_CAN1_Send_EXT(uint32_t id, uint8_t length, const uint8_t data[64])
{
    ...
    do {
        status = CAN_GetTransferStatus(&can1_instance, TX_MAILBOX_CAN1_EXT);
        if (status == STATUS_SUCCESS) {
            if (CAN_Send(&can1_instance, TX_MAILBOX_CAN1_EXT, &Tx_msg) == STATUS_SUCCESS) {
                return ;
            }
        }
    } while (++timeout < 500);

    if (CAN_AbortTransfer(&can1_instance, TX_MAILBOX_CAN1_EXT) != STATUS_SUCCESS) {
        return;
    }
...
void CAN1_Callback(uint32_t instance, can_event_t event, uint32_t buffIdx, void *flexcanState)
{
	...
	CAN_Receive(&can1_instance, RX_MAILBOX_CAN1_STD, &recvMsg_CAN_STD);
	CAN_Receive(&can1_instance, RX_MAILBOX_CAN1_EXT, &recvMsg_CAN_EXT);
	...
	switch(event)
	{
		case CAN_EVENT_RX_COMPLETE:
			...
			if(recvMsg_CAN_STD.id != 0)
			{
			    ...
			    xQueueSendFromISR(g_can1_rx_queue, &temp_msg, 0);
			}
...
void BSP_CAN1_Init(void)
{
	...
	CAN_ConfigTxBuff(&can1_instance, TX_MAILBOX_CAN1_STD, &Tx_buffCfg_STD);

	CAN_ConfigRxBuff(&can1_instance, RX_MAILBOX_CAN1_EXT, &Rx_buffCfg_EXT, Rx_Filter);
	CAN_ConfigTxBuff(&can1_instance, TX_MAILBOX_CAN1_EXT, &Tx_buffCfg_EXT);

	CAN_SetRxFilter(&can1_instance, CAN_MSG_ID_STD, RX_MAILBOX_CAN1_STD, 0x0000);
	CAN_SetRxFilter(&can1_instance, CAN_MSG_ID_EXT, RX_MAILBOX_CAN1_EXT, 0x00000000);
	CAN_InstallEventCallback(&can1_instance, &CAN1_Callback, (void*)0);
